...
$users=Get-ADUser –SearchBase “OU=User Accounts,DC=hq,DC=icfed,DC=com” -filter {(whencreated -ge $lastday) -and (Enabled -eq $True) } | select samaccountname


#$users=Get-ADUser –SearchBase “OU=User Accounts,DC=hq,DC=icfed,DC=com” -filter {(Enabled -eq $True)} | select samaccountname
foreach ($User in $Users){
$sam=$user.samaccountname
$HomeDir ='\\hq.icfed.com\dfs\users data$\users data\{0}' -f $sam; #change it with your servername and share
