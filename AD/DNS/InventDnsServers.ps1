...
$ResultPatch = "C:\TEMP\Servers_$domain-$filedate.txt"

$ResultPingtPatch = "C:\TEMP\ServersPing_$domain-$filedate.txt"
...
$ResultPatch = "\\boev.hq.icfed.com\logs\Servers_.txt"
...
# get-dns
Get-NetIPConfiguration -All -Detailed | Where-Object {$_.IPv4DefaultGateway -ne $null} | foreach {$_.IPv4Address} | select IPAddress 
 
"DNS" 
$num=Get-NetIPConfiguration -All -Detailed | Where-Object {$_.IPv4DefaultGateway -ne $null} | FOREACH{$_.InterfaceIndex}
foreach($n in $num){Get-DnsClientServerAddress -InterfaceIndex $n |foreach{$_.ServerAddresses }}
